<template>
  <section>
    <ul class="comment-list">
      <li 
        v-for="({ author, comment, authorStatus, avatar }, index) in commentList" 
        :key="index"
        class="comment-item"
        :style="{ transform: `translateY(${1 * index}rem)`}"
      >
        <div>
          <header>
            <img 
